...
func TestChecksum(t *testing.T) {
	...
	t.Run("sha256sum single file", func(t *testing.T) {
		...
		afero.WriteFile(fs, "/hello.txt", content, 0o644)
	...
	t.Run("multiple files", func(t *testing.T) {
		fs := afero.NewMemMapFs()
		afero.WriteFile(fs, "/a.txt", []byte("hello"), 0o644)
		afero.WriteFile(fs, "/b.txt", []byte("world"), 0o644)
		var buf strings.Builder
		s := NewTestShell(t, &buf, shell.WithFS(fs))
	...
	t.Run("check mode passes", func(t *testing.T) {
		...
		afero.WriteFile(fs, "/sums.txt", []byte(sumLine), 0o644)
		var buf strings.Builder
		s := NewTestShell(t, &buf, shell.WithFS(fs))
	...
	t.Run("check mode fails on mismatch", func(t *testing.T) {
		...
		afero.WriteFile(fs, "/badsums.txt", []byte(sumLine), 0o644)
		var buf strings.Builder
		s := NewTestShell(t, &buf, shell.WithFS(fs))
