...
- [x] **Move Ruby-specific WASM hacks into plugin config**
...
- [x] **Replace bubble sort in `session.Store.List()`**
...
- [x] **Fix unbounded linear scan in `allocFd`**
...
- [x] **Document plugin loading priority inside `plugin.go`**
...
- [x] **Reduce repetitive error handling in `composeGraph`**
...
- [x] **Add tests for `internal/agent/`**
...
- [x] **Give `session.Store.reap()` a shutdown path**
...
- [x] **Validate conflicting `WithDisabledPlugins` and `WithPluginFilter` options**
...
- [x] **Group entries in `defaultNativePlugins()` with section comments**
